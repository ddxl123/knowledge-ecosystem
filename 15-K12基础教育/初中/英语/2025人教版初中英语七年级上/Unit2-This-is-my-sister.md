# Unit 2: This is my sister

## 重点词汇
- sister /ˈsɪstər/ 姐妹
- mother /ˈmʌðər/ 母亲
- father /ˈfɑːðər/ 父亲
- parent /ˈpeərənt/ 父（母）亲
- brother /ˈbrʌðər/ 兄弟
- grandmother /ˈɡrænmʌðər/ （外）祖母
- grandfather /ˈɡrænfɑːðər/ （外）祖父
- grandparent /ˈɡrænpeərənt/ （外）祖父母
- family /ˈfæməli/ 家庭
- those /ðəʊz/ 那些
- these /ðiːz/ 这些
- who /huː/ 谁
- they /ðeɪ/ 他（她、它）们
- well /wel/ 嗯；好吧
- have /hæv/ 经受；经历
- day /deɪ/ 一天；一日

## 核心句型
1. This is my sister. 这是我的姐姐。
2. That is my brother. 那是我的哥哥。
3. These are my parents. 这些是我的父母。
4. Those are my grandparents. 那些是我的祖父母。
5. — Who's she? — She's my sister. 她是谁？她是我姐姐。
6. — Who're they? — They're my grandparents. 他们是谁？他们是我的祖父母。

## 语法要点

### 指示代词
| 单数 | 复数 |
|------|------|
| this 这个 | these 这些 |
| that 那个 | those 那些 |

### 人称代词（主格）
| 单数 | 复数 |
|------|------|
| I | we |
| you | you |
| he/she/it | they |

### 名词复数规则变化
1. 一般加 -s: sister → sisters
2. 以s/x/ch/sh结尾加 -es: box → boxes
3. 以辅音+y结尾，变y为i加 -es: family → families
4. 以f/fe结尾，变f/fe为v加 -es: knife → knives

## 家庭成员词汇扩展
- son /sʌn/ 儿子
- daughter /ˈdɔːtər/ 女儿
- cousin /ˈkʌzn/ 堂（表）兄弟姐妹
- uncle /ˈʌŋkl/ 叔叔；伯伯
- aunt /ɑːnt/ 阿姨；姑姑

## 对话范例
A: Good morning, Jane!
B: Good morning, Sally!
A: Oh, Jane, this is my sister Kate. Kate, this is my friend Jane.
B: Nice to meet you, Jane.
C: Nice to meet you, too. Are those your parents?
B: Yes, they are.

## 收集指南

> ⚠️ **严格范围限定：** 仅收集本文件内容，禁止跨文件、跨目录引用。例如本文件为「Unit2-This-is-my-sister」，则只收集该单元内容，不得涉及其他单元

### 概述

**Unit 2: This is my sister** 是英语学科中的语言知识模块，涉及词汇、语法、听说读写等语言技能。

### 知识结构

本部分内容按以下结构组织：

1. **重点词汇**
2. **核心句型**
3. **语法要点**
4. **家庭成员词汇扩展**
5. **对话范例**

### 关键概念

- parent /ˈpeərənt/ 父（母）亲
- grandmother /ˈɡrænmʌðər/ （外）祖母
- grandfather /ˈɡrænfɑːðər/ （外）祖父
- grandparent /ˈɡrænpeərənt/ （外）祖父母
- they /ðeɪ/ 他（她、它）们

### 学习要点

- 在语境中学习词汇和语法
- 注重听说读写的综合训练
- 多读多听，培养语感
